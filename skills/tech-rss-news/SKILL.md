...
description: |
  ...
  【硬性约束】运行 fetch_ai_news / fetch_dev_news / fetch_sec_news 后，对用户回复 MUST 保留脚本输出中每一个 https:// 或 http:// 完整 URL；NEVER 删除或概括掉链接行；NEVER 用无 URL 的 bullet 列表替代脚本全文；文末「原文链接汇总」MUST 原样转发。若需缩写，仅删除无 URL 的句子，不得减少 URL 条数。
...
本技能提供三条独立脚本，共用 `skills/shared` 下的格式化与过滤模块：

| 脚本 | 用途 |
|------|------|
| `fetch_ai_news.py` | AI/技术资讯（含 AI 关键词过滤） |
| `fetch_dev_news.py` | 编程语言、开源/产品发布、开发者社区与热点 |
| `fetch_sec_news.py` | 网络安全、漏洞/CVE（含 NVD/CISA JSON）、威胁情报 |
...
**NVD JSON**：部分网络环境对 `nvd.nist.gov` 返回 403，可设置环境变量 `OPENCLAW_NVD_CVE_GZ_URL` 指向可访问的 `nvdcve-1.1-recent.json.gz` 镜像地址。
