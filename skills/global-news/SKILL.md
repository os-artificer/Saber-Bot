...
description: |
  ...
  【硬性约束·违反视为错误输出】
  (1) 运行 fetch_global_news.sh（先 Python，失败再 bash）后，对用户可见的回复中 MUST 保留脚本 stdout 里每一个含 https:// 或 http:// 的完整行；NEVER 删除、省略、改写、概括掉任何 URL 字符串。
  ...
  脚本路径：~/.openclaw/skills/global-news/scripts/fetch_global_news.sh
---
...
| 必须（MUST） | 禁止（MUST NOT） |
|----------------|------------------|
| 执行 `fetch_global_news.sh` 后，把**终端打印的完整文本**发给用户（可拆多条消息） | 把脚本输出**改写**成「突发 / 中东 / 国内要闻」等**自编专题**，再只写要点不要链接 |
...
**统一入口**为 `fetch_global_news.sh`：**优先**执行同目录下的 `fetch_global_news.py`（`python3`）；若 Python 进程**非零退出**（失败），或系统**无 `python3`/脚本不存在**，则**自动**执行 `fetch_global_news.bash.sh`（纯 bash + `curl` / `jq` 等）。Agent **只需**调用下面这一条，无需手选 Python 或 bash。

```bash
bash ~/.openclaw/skills/global-news/scripts/fetch_global_news.sh [天数] [分类]
```

- **Python 路径**：依赖 `skills/shared` 下的模块（与仓库布局一致时可直接运行）。
- **Bash 回退**：需 `curl`、`jq`、`date`、**GNU grep**（`-P`）；若已安装 **libxml2** 的 `xmllint`，RSS 优先用其解析，否则为 `grep -Pzo` 粗解析（大文件会先截断至约 4MB）。
...
```bash
# 今日国内+国际综合资讯
bash fetch_global_news.sh

# 近3天国际局势（中文源 + 国际主流源）
bash fetch_global_news.sh 3 world

# 近7天科技
bash fetch_global_news.sh 7 tech

# 国内动态（国内源）
bash fetch_global_news.sh 0 domestic

# 国际媒体里的中国相关新闻
bash fetch_global_news.sh 3 china
```
...
每条资讯包含：**优先一行裸 `https://` 链接**（便于 IM 识别可点击）、摘要、时间、**`[阅读原文](url)`**（Markdown 可点击）、`链接 ·` 行与再次重复的裸 URL；输出分为「国内与中文源」和「国际主流源」两块（按分类自动显示）。

## 给 Agent 的硬性规则（不可省略）

1. **禁止**用自编的「突发 / 中东局势 / 国内要闻 / 国际其他」等专题卡片**替换**脚本输出；那种版式会丢掉全部 `https://` 链接。
2. 用户要全球/国内/国际资讯时，**必须**运行 `fetch_global_news.sh`，将**脚本打印的原文**（含每条 `https://` 与 `[阅读原文](…)`）发给用户；可分段发送，**不可**只发归纳标题。
